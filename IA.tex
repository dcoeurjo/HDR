...
noterons   $\sI{x}=  \fup{x}$  et  $\iI{x}=\fdown{x}$  les  opérateurs
...
choisit un  \emph{mode  d'arrondi} selon l'opérateur $\fup{\cdot}$  ou
...
Remarquons  l'usage des opérateurs  $\fup{\cdot}$  et  $\fdown{\cdot}$
...
\begin{mapreuve}
  ...
  $[\fdown{f(x)},\fup{f(x)}] \subseteq Y$.   Par définition du modèle en
  supercouverture,         $C$ étant             défini            par
  $[k-\frac{1}{2},k+\frac{1}{2}]\times[\fdown{f(x)},\fup{f(x)}]$, nous
